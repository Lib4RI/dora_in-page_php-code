...
$link = url("/stats.dl", array('absolute' => true)) . "?year=2020";
...
foreach( $colAry as $colid => $colName ) {
	...
	$result = $query->fields('dses', array('dsid'))
		->groupBy('dsid')
		->execute()
		->fetchAllKeyed();
	...
	$dlSum = 0;
	foreach($result as $dsId => $dlNum ) {
		if ( !strpos($dsId,'_') && stripos($dsId,'PDF') === 0 ) { $dlSum += $dlNum; }
	}

	echo "<br>For {$inst} totally <b> {$dlSum} </b> downloads of 'PDF...' datastreams ";
